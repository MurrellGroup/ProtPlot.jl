...
const DEFAULT_COLORSCHEME = :gist_rainbow
...
# does ugly twists if derivative is not constant
function tube(
    points::AbstractMatrix{T},
    radius::Real = 0.5;
    spline_quality = 10,
    ...
    color_matrix = repeat(colorscheme[LinRange(1-color_start, 1-color_end, size(surface_vertices, 2))], inner=(spline_quality, tube_quality))
...
function sheet_surface(
    ...
    #lines!.([path1, path2], color=:white, linewidth=10)
...
function sheet(
    ...
    colorscheme = colorscheme isa ColorScheme ? colorscheme : colorschemes[colorscheme]
    color_matrix = repeat(colorscheme[LinRange(1-color_start, 1-color_end, size(surface_vertices, 2))], inner=(spline_quality, 5))
    return surface_vertices, color_matrix
end
...
function Makie.plot!(ramachandran::Ramachandran{<:Tuple{<:AbstractVector{<:Real}, <:AbstractVector{<:Real}}})
    phi_angles = @lift rad2deg.($(ramachandran.phi_angles))
    psi_angles = @lift rad2deg.($(ramachandran.psi_angles))
    scatter!(ramachandran, phi_angles, psi_angles, color=ramachandran.color)
    return ramachandran
end
...
function Makie.convert_arguments(::Type{<:Ramachandran}, chain::ProteinChain)
    ...
    torsion_angles = get_torsion_angles(chain)
